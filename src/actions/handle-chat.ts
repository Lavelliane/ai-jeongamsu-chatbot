...
export async function handleChat(input: string, sessionId: string) {
  ...
  const coinFlip = Math.floor(Math.random() * 2) === 0 ? 0 : 1;
  const shouldHaveImage = coinFlip === 0 ? `<br /><br /><img style="width: 60%; height: auto; margin: auto;" src="/jeonghamsu-icons/${Math.floor(Math.random() * 15) + 1}.svg" alt="emoji" />` : '';
